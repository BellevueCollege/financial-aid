...
Class Form_Post_Controller extends Faform_Controller{
            ...
            const COLLEGE_DEGREE = "Please check Yes for previously attended college.";
            const TYPE_OF_DEGREE = "Choose the type(s) of degree(s) you hold.";
            const DEGREE_SELECTION = "Please make sure that previously attended college and do you hold a college degree fields are marked Yes.";
            ...
            const FUNDING = "Please make sure that  'Do you expect to receive another source of funding ...'  field is marked Yes.";
            ...
            const LOANS = "Please make sure that 'Would you like to apply for financial aid loans' field is marked Yes.";
...
function validate_form()
{
        ...
        else
        {
                ...
                */
                ...
                $errors = $this->validate_degree($errors);
                ...
                $errors = $this->validate_funding($errors);

                $errors = $this->validate_loans($errors);
...
function validate_degree($errors)
{
 // Start of Previous Schools validation section
    ...
    else if($this->model->get_attend_college_pv() == '1')
    {
            ...
            else if($this->model->get_hold_college_degree_pv() == '0') 
            {
                if($this->model->get_type_of_degree_pv() != NULL )
                {
                    $errors[] = self::DEGREE_SELECTION;
                }
            }
    }
    else if($this->model->get_attend_college_pv() == '0')
    {
        if($this->model->get_hold_college_degree_pv() == '1') 
        {
            $errors[] =  self::COLLEGE_DEGREE;                                        		
        } 
        if($this->model->get_type_of_degree_pv() != NULL )
        {
            $errors[] = self::DEGREE_SELECTION;
        }
    }// End of Previous Schools validation section
    return $errors;
} // end of validate_degree
function validate_funding($errors)
{
    // Start Validate Third Party Funding section
   ...
   else if($this->model->get_third_party_funding_pv() == '0' )
   {
       if($funding_amt != null || $funding_source_value != null)
       {
           $errors[] = self::FUNDING;
       }
   }
...
// End Validate Third Party Funding section
}// end of validate_funding

function validate_loans($errors)
{
 // Start Validate Loans section                              
    ...
    else if($this->model->get_apply_for_fa_pv() == '0')
    {
        if($this->model->get_types_of_loan_pv() != null || $this->model->get_require_loan_quarters_pv() != null) 
                $errors[] = self::LOANS;
    }
    return $errors;
// End Validate Loans Section
}// end of validate_loans
